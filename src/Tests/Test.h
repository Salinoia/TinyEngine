...
// 全局测试注册表
struct TestRegistry {
    static inline std::vector<std::pair<std::string, std::function<std::unique_ptr<Test>()>>>
        s_Tests;
};
...
public:
    ...
    static void RegisterTest(const std::string& name) {
        ...
        TestRegistry::s_Tests.push_back(
            std::make_pair(name, []() { return std::make_unique<T>(); }));
    }
    ...
    static void RegisterTest(const std::string& name, Args&&... args) {
        std::cout << "Registering Test: " << name << std::endl;
        TestRegistry::s_Tests.push_back(std::make_pair(
            name, [=]() mutable { return std::make_unique<T>(std::forward<Args>(args)...); }));
    }
